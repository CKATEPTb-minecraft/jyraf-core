...
import static dev.ckateptb.minecraft.jyraf.packet.entity.meta.MetaOffsetConverter.EntityMetaOffsets.*;
...
public class EntityMeta implements EntityMetadataProvider {
    ...
    protected static void isVersionNewer(ServerVersion version) {
        if (!Jyraf.getPlugin().getPacketApi().getServerManager().getVersion().is(VersionComparison.NEWER_THAN, version)) {
            throw new RuntimeException("This method is only available for versions newer than " + version.name() + ".");
        }
    }

    protected static boolean isVersion(ServerVersion version, VersionComparison comparison) {
        return Jyraf.getPlugin().getPacketApi().getServerManager().getVersion().is(comparison, version);
    }

    protected static boolean isVersion(ServerVersion version) {
        return Jyraf.getPlugin().getPacketApi().getServerManager().getVersion().is(VersionComparison.EQUALS, version);
    }
